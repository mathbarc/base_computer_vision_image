...
RUN apt update;DEBIAN_FRONTEND=noninteractive apt install cmake g++ make git libavcodec-dev libavformat-dev libavresample-dev libswscale-dev libeigen3-dev -y; apt clean --dry-run; apt autoclean;

WORKDIR /home
RUN git clone https://github.com/davisking/dlib.git --branch v19.22 --single-branch; mkdir /home/dlib/build; cd /home/dlib/build; cmake .. -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_INSTALL_PREFIX=/usr/local; make -j 1 install; cd ../..; rm -rf dlib; ldconfig
