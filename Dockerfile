...
RUN apt update; DEBIAN_FRONTEND=noninteractive apt install libavcodec59 libavformat59 libswresample4 libswscale6 libtesseract5 libopenni2-0 libusb-1.0-0 libgdal32 libgdcm3.0 libpython3-dev python3-pip -y; apt clean --dry-run; apt autoclean; pip3 install numpy --break-system-packages; 

FROM base as builder

RUN DEBIAN_FRONTEND=noninteractive apt install cmake g++-11 make git libavcodec-dev libavformat-dev libswresample-dev libswscale-dev libeigen3-dev libtesseract-dev libopenni2-dev libopenni-dev libusb-1.0-0-dev libgdal-dev libgdcm-dev libpython3-dev python3-pip -y; apt clean --dry-run; apt autoclean; pip3 install numpy --break-system-packages; 
